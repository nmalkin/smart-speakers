...
describeOutsideCI('test in browser', () => {
    ...
    describe('survey flow', () => {
        ...
        test('recording check page shows retry button', async () => {
            // Provide consent
            await page.waitForSelector(consentChoiceSelector);
            await page.click(consentChoiceSelector);
            await clickNext(page);
...
function getCredentials(device: Device): Credentials {
    try {
        ...
        throw new Error(`unable to find and extract ${device} credentials`);
    }
}
